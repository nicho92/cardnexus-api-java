...
public class NexusWizardPanel extends JPanel {
	...
	public NexusWizardPanel() {
		...
		btnAddProduct.addActionListener(_->{
			...
			try 
			{
			    entry.setMaxUnitPrice(new Amount(c.getPricesByFinish().get(c.getFinishes().getFirst()).cardmarket().low(), currency));
			}
			catch(Exception _)
			{
			...
			try 
			{
			    entry.setMaxUnitPrice(new Amount(c.getPrices().cardmarket().low(), currency));
			}
			catch(Exception _)
			{
			    entry.setMaxUnitPrice(new Amount(1.0,currency));
